...
if [[ $APP = "controller-manager" ]];then
    kubectl patch deployments.apps -n volcano-system volcano-controllers --patch "{\"spec\": {\"template\": {\"spec\": {\"containers\": [{\"name\": \"volcano-controllers\",\"image\": \"${ImageName}\"}]}}}}"
fi